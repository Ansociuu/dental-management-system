...
const AppointmentBooking = () => {
  ...
  const shiftColors = ['from-blue-500 to-blue-600', 'from-amber-500 to-amber-600', 'from-purple-500 to-purple-600'];
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        ...
        <div className="xl:col-span-3">
            ...
            {calLoading ? (
              ...
              <div className="grid grid-cols-7">
                {calendarDays.map((cell, idx) => {
                  ...
                  return (
                    <div key={idx} className={`min-h-[100px] border-b border-r border-slate-100 p-1 relative ${cell.currentMonth ? 'bg-white' : 'bg-slate-50/60'} ${today ? 'ring-2 ring-inset ring-blue-400/40' : ''} ${isPast ? 'opacity-50' : ''}`}>
                      <div className="text-right mb-0.5 pr-0.5">
                          ...
                          return group.doctors.map(duty => {
                            ...
                            return (
                              <button key={duty._id} type="button" disabled={isPast || isFull}
                                onClick={() => !isPast && !isFull && handleSlotClick(cell.date, group.shift._id, duty.doctorId?._id)}
                                className={`w-full text-left px-1 py-0.5 rounded text-[8px] font-bold truncate border transition-all ${
          ...
          <div className="mt-3 bg-white rounded-xl border border-slate-100 px-4 py-3 flex flex-wrap items-center gap-4 text-[10px] font-bold text-slate-500">
            ...
            <span className="text-slate-400">💡 Nhấp vào ca trực để tự động điền form</span>
          </div>
        </div>
      </div>
    </div>
